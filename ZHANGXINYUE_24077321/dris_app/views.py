...
def view_shelters(request):
    ...
    for shelter in shelters:
        if shelter.capacity > 0:
            shelter.occupancy_percentage = (shelter.current_occupancy / shelter.capacity) * 100
        else:
            shelter.occupancy_percentage = 0
...
def volunteer_dashboard(request):
    ...
    completed_count = assignments.filter(is_completed=True).count()
    pending_count = assignments.filter(is_completed=False).count()
...
def authority_dashboard(request):
    ...
    # 计算不可用庇护所数量
    unavailable_shelters = total_shelters - available_shelters
    ...
    context = {
        ...
        'unavailable_shelters': unavailable_shelters,  # 添加这个变量
    }
